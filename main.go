...
Init Command:
  ...
  polaris init --skip-skills           Skip skill installation
...
func checkInstalledSkills() {
	...
	installedVersion := getInstalledSkillsVersion(targetDir)
...
// skillsAPIResponse is the response from GET /api/v1/skills
type skillsAPIResponse struct {
	Version string         `json:"version"`
	Skills  []skillFromAPI `json:"skills"`
}

type skillFromAPI struct {
	Name     string `json:"name"`
...
func downloadAndInstallSkills(force, yesAll bool) (installed bool, ver string, err error) {
	home, err := os.UserHomeDir()
	if err != nil {
	...
	if loadErr != nil || cfg == nil || cfg.APIKey == "" || cfg.APIURL == "" {
		// No credentials configured — check if skills are already installed
		if _, statErr := os.Stat(targetDir); statErr == nil {
			installedVersion := getInstalledSkillsVersion(targetDir)
			fmt.Printf("Skills: Using locally installed version (%s)\n", installedVersion)
			fmt.Println("  Run 'polaris login' to enable skill updates from the server.")
	...
	client := &http.Client{Timeout: 30 * time.Second}
	req, reqErr := http.NewRequest("GET", cfg.APIURL+"/api/v1/skills", nil)
	if reqErr != nil {
	...
	if doErr != nil {
		...
		if _, statErr := os.Stat(targetDir); statErr == nil {
			installedVersion := getInstalledSkillsVersion(targetDir)
			fmt.Printf("Skills: Could not reach server, using local version (%s)\n", installedVersion)
			return true, installedVersion, nil
		}
	...
	if resp.StatusCode != 200 {
		...
		if _, statErr := os.Stat(targetDir); statErr == nil {
			installedVersion := getInstalledSkillsVersion(targetDir)
			fmt.Printf("Skills: Server returned %d, using local version (%s)\n", resp.StatusCode, installedVersion)
			return true, installedVersion, nil
		}
		return false, "", fmt.Errorf("fetch skills failed (status %d): %s", resp.StatusCode, strings.TrimSpace(string(body)))
	}

	var apiResp skillsAPIResponse
	if err := json.NewDecoder(resp.Body).Decode(&apiResp); err != nil {
		return false, "", fmt.Errorf("decode skills response: %w", err)
	}

	serverVersion := apiResp.Version

	// If the target is a symlink (old install model), remove it
	if linkTarget, linkErr := os.Readlink(targetDir); linkErr == nil {
		fmt.Printf("Replacing skills symlink (%s) with downloaded skills\n", linkTarget)
		if err := os.Remove(targetDir); err != nil {
			return false, "", fmt.Errorf("cannot remove existing symlink: %w", err)
		}
	}

	// Check if skills already exist and compare versions
	if info, statErr := os.Stat(targetDir); statErr == nil && info.IsDir() {
		installedVersion := getInstalledSkillsVersion(targetDir)

		if installedVersion == serverVersion && !force {
			fmt.Printf("Skills: Up to date (v%s)\n", serverVersion)
		...
		if installedVersion != "" && installedVersion != serverVersion && !force {
			if yesAll {
				...
				upgradeErr := huh.NewConfirm().
					Title(fmt.Sprintf("Upgrade skills from v%s to v%s?", installedVersion, serverVersion)).
					Affirmative("Yes").
					Negative("No").
					Value(&upgrade).
					Run()
				if upgradeErr != nil || !upgrade {
					fmt.Println("Skills: Keeping existing version")
					return true, installedVersion, nil
				}
			}
		}
	...
	if err := os.MkdirAll(targetDir, 0755); err != nil {
		return false, "", fmt.Errorf("cannot create skills directory: %w", err)
	}

	// Write each skill file
	var skillNames []string
	for _, skill := range apiResp.Skills {
		// Prepend version header
		versionHeader := fmt.Sprintf("<!-- polaris-version: %s -->\n", serverVersion)
		content := versionHeader + skill.Content

		targetPath := filepath.Join(targetDir, skill.Filename)
	...
	fmt.Printf("Skills: Installed %d skills (v%s) to %s\n", len(skillNames), serverVersion, targetDir)
	fmt.Printf("  Available: %s\n", strings.Join(skillNames, ", "))

	return true, serverVersion, nil
}

// getInstalledSkillsVersion reads the version from an installed skill file
func getInstalledSkillsVersion(dir string) string {
